    ...
    <section className="page-shell case-detail">
      ...
      <div className="case-detail-grid">
        ...
        <aside className="case-side">
          ...
          {r.subcategory && <p><strong>{t.submitPage.subcategory}</strong><span><Translatable text={r.subcategory} /></span></p>}
          {r.audience && <p><strong>{t.filters.audience}</strong><span><Translatable text={r.audience} /></span></p>}
          ...
          {locationLabel && <p><strong>{t.caseDetail.location}</strong><span><Translatable text={locationLabel} /> · <Translatable text={locationParts} /></span></p>}
          {r.eventDate && <p><strong>{t.caseDetail.date}</strong><span>{r.eventDate}</span></p>}
          {r.timeLimitType && <p><strong>{t.caseDetail.timeLimit}</strong><span><Translatable text={r.timeLimitType} /></span></p>}
          ...
          {r.duration && <p><strong>{t.caseDetail.lectureDuration}</strong><span><Translatable text={r.duration} /></span></p>}
          {r.format && <p><strong>{t.caseDetail.format}</strong><span><Translatable text={r.format} /></span></p>}
          {steps.length > 0 && (
              ...
              {steps.map((step) =>
                step.files.length > 0 && (
                  <div key={step.id} className="side-step-files">
                    <span><Translatable text={step.text} /></span>
                    {step.files.map((f) => (
